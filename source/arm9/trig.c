...
2669640,	5340085, MAX_TAN
	...
	fixed_s32_15 sinLutLookup(int i)
	{	
		i = (i >> ANG_BITSFRACTION) & 511;

		int lutVal = i & 127;

		if(i < 128) return (fixed_s32_15)(SIN_LUT[lutVal]);
		if(i < 256) return (fixed_s32_15)(SIN_LUT[(LUT_SIZE - lutVal)]);
		if(i < 384) return -(fixed_s32_15)(SIN_LUT[lutVal]);
	    return -(fixed_s32_15)(SIN_LUT[(LUT_SIZE - lutVal)]);
	}
	...
	fixed_s16_12 sinLerp(s16 angle){
		fixed_s32_15 prev = sinLutLookup(angle);
		fixed_s32_15 next = sinLutLookup(angle + intToFixed(1, ANG_BITSFRACTION));
		fixed_s32_15 interp = angle & ((1 << ANG_BITSFRACTION) - 1);
	...
	fixed_s32_16 tanLutLookup(int i)
	{
		i = (i >> ANG_BITSFRACTION) & 511;
	...
	int tanLerp(s16 angle)
	{
		fixed_s32_16 prev;
		
		int lut_val = (angle >> ANG_BITSFRACTION) & 511;
		...
		fixed_s32_16 next = tanLutLookup(angle + intToFixed(1, ANG_BITSFRACTION));
		fixed_s32_16 interp = angle & ((1 << ANG_BITSFRACTION) - 1);

		return (int)
			((prev + (((next-prev) * interp) >> ANG_BITSFRACTION)) >> (TAN_BITSFRACTION - 12));
	}

	int arcSinComp(const void *a, const void *b) {
		short* par = (short*)a;
		short* lut = (short*)b;

		if(*par == *lut || (*lut < *par && *par < *(lut+1))){
			return 0;
		}
		if(*par < *lut){
			return -1;
	...
	short asinLerp(int par)
	{
		...
		short* as = (short*)bsearch(&par, SIN_LUT, LUT_SIZE, sizeof(short), arcSinComp);
	...
	{
		short aSin = asinLerp(par); /* Uses the identity sin(x) = cos(90 - x) => asin(x) = 90 - acos(x) */

		return intToFixed(128, ANG_BITSFRACTION) - aSin; /* returns a value in [0, 256] */
	}

	//todo: implement
	//short atanComp(const void *a, const void *b) {
	//	f32* par = (f32*)a;
	//	f32* lut = (f32*)b;
	...
	//	f32* at = (f32*)bsearch(&par, TAN_LUT, LUT_SIZE, sizeof(f32), arcTanComp);
